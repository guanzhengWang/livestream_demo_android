...
public class StartLiveActivity extends LiveBaseActivity
        ...
        if (user != null) {
            NetDao.createLive(StartLiveActivity.this, user, new OnCompleteListener<String>() {
                ...
                public void onError(String error) {
                    pd.dismiss();
                    CommonUtils.showLongToast("创建直播失败" + error);
                }
            });
        } else {
            pd.dismiss();
            CommonUtils.showLongToast("当前用户获取失败");
        }
    }
    ...
    private void showConfirmCloseLayout(String time) {
        ...
        EaseUserUtils.setUserAvatar(StartLiveActivity.this, EMClient.getInstance().getCurrentUser(), coverImage);
   /* List<LiveRoom> liveRoomList = TestDataRepository.getLiveRoomList();
    for (LiveRoom liveRoom : liveRoomList) {
      if (liveRoom.getId().equals(liveId)) {
        ...
        TextView showTimeView = (TextView) view.findViewById(R.id.tv_showTime);
        EaseImageView userAvatar = (EaseImageView) view.findViewById(R.id.eiv_avatar);
